...
# * Suggest a new taxon it might be able to be moved to. This is done by finding content in each taxon that
# it has a a cosine similarity score of <= 0.5 and finding the mean. The taxon with the lowest mean score
# is considered the best. The tree is traversed in a depth first manner, so the taxon with the best score
# is selected and then it's children are compared before finding the best and continuing down.
# This resulted in fewer wildly inaccurate suggestions.
